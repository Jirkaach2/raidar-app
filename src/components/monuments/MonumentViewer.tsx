...
type TierFilter = 'all' | 't1' | 't2' | 't3' | 'safe';
...
export function MonumentViewer() {
  ...
  const filtered = useMemo(() => {
    ...
    return monuments.filter((m) => {
      if (!matchesTier(m, tier)) return false;
      if (q && !m.name.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [monuments, query, tier]);
    ...
    <div className="mon-panel">
      ...
      <div className="mon-body">
        ...
        <aside className="mon-list-pane">
          ...
          <div className="mon-chips">
            {chips.map((c) => (
              <button
                key={c.id}
                className={`mon-chip ${tier === c.id ? 'mon-chip--active' : ''}`}
        ...
        <section className="mon-detail">
          {selected ? <MonumentDetail info={selected} /> : <div className="mon-list-empty">Select a monument.</div>}
        </section>
      </div>
    </div>
  );
...
function MonumentDetail({ info }: { info: MonumentInfo }) {
  const radText =
